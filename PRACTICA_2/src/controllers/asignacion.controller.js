...
function agregarAsignacion(req, res) {
    ...
    if (req.user.rol == 'Rol_Alumno') {
        if (parametros.idCursoMaestro && parametros.idAlumnoCurso) {
            ...
            modeloAsignaciones.maxAsignaciones + 1;

            if (parametros.maxAsignaciones < 3) {
...
function eliminarAsignacion(req, res) {
    ...
    Cursos.findByIdAnsDeletar(idCurso, (err, asignacionesEliminados) => {
        if (err) return res.status(500).send({ mensaje: 'Error en la Peticion' });
        if (!asignacionesEliminados) return res.status(500).send({ mensaje: 'Error al Eliminar' });

        return res.status(200).send({ asignacion: asignacionesEliminados });
    })
}
